...
def criterion(criterion_id: str, passed: bool, description: str, *, expected: str | None = None, observed: str | None = None) -> dict[str, str]:
    row = {"criterion_id": criterion_id, "status": "pass" if passed else "fail", "description": description}
...
def relation_gate(criterion_id: str, spec: dict[str, Any], text: str) -> dict[str, str]:
    missing = [pattern for pattern in spec["patterns"] if re.search(pattern, text, re.IGNORECASE | re.DOTALL) is None]
    return criterion(
        criterion_id,
        not missing,
...
def evaluate(candidate_root: Path) -> dict[str, Any]:
    ...
    rows.append(criterion("ledger_heading", ledger.startswith(CONFIG["ledger_title"] + "\n"), "exact task-native ledger heading", expected=CONFIG["ledger_title"], observed=ledger.splitlines()[0] if ledger.splitlines() else ""))
    rows.append(criterion("ledger_source_breadth", len(citations(ledger)) >= CONFIG["minimum_ledger_sources"], f"{len(citations(ledger))} distinct cited sources"))
    rows.append(criterion("decision_title", decision.startswith(CONFIG["decision_title"]), "exact decision title", expected=CONFIG["decision_title"], observed=decision.splitlines()[0] if decision.splitlines() else ""))
    headings = re.findall(r"(?m)^## ([^\r\n]+)\s*$", decision)
    rows.append(criterion("decision_heading_order", headings == CONFIG["decision_headings"], "exact ordered level-two headings", expected=" | ".join(CONFIG["decision_headings"]), observed=" | ".join(headings)))
    ...
    rows.append(criterion("decision_source_breadth", len(citations(decision)) >= CONFIG["minimum_unique_sources"], f"{len(citations(decision))} distinct cited sources"))
    ...
    for gate, patterns in CONFIG["forbidden_relation_patterns"].items():
        ...
        rows.append(criterion(gate, not present, "no prohibited relation conversion" if not present else "prohibited: " + ", ".join(present), expected="none", observed=", ".join(present) if present else "none"))
    for gate, patterns in CONFIG["negation_aware_forbidden_patterns"].items():
        present = [match for pattern in patterns for match in unnegated_occurrences(combined, pattern)]
        rows.append(criterion(gate, not present, "no unnegated prohibited relation conversion" if not present else "prohibited: " + ", ".join(present), expected="none", observed=", ".join(present) if present else "none"))

    blocking = [f"{row['criterion_id']}: {row['description']} | expected={row.get('expected', '')} | observed={row.get('observed', '')}" for row in rows if row["status"] != "pass"]
    passed = not blocking
    return {
        "schema_version": "orchard-biologics-evaluation-result-v0",
