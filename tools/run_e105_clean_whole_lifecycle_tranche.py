...
STAGE0 = ROOT / "TRELLIS_CLEAN_WHOLE_LIFECYCLE_STAGE0.json"
...
def external_evaluation(candidate_root: Path, output_path: Path) -> dict[str, Any]:
    ...
    receipt = {
        ...
        "schema": "trellis-clean-lifecycle-checkpoint-evaluation-v0",
...
def main() -> int:
    ...
    if stage0.get("passed") is not True or stage0.get("live_authorized") is not False:
        raise RuntimeError("frozen Stage 0 is not qualified and unauthorized")
    ...
    write_json(
        run_root / "FREEZE_BINDING.json",
        {
            ...
            "run_id": RUN_ID,
            "schema": "trellis-clean-whole-lifecycle-freeze-binding-v0",
    ...
    try:
        ...
        if tranche.actor_attempts > INITIAL_MAXIMUM_ACTOR_CALLS:
            raise RuntimeError("actor call authorization exceeded")
        if tranche.maintenance_attempts > INITIAL_MAXIMUM_MAINTENANCE_CALLS:
            raise RuntimeError("maintenance call authorization exceeded")
        if tranche.counters.provider_attempts > INITIAL_MAXIMUM_PROVIDER_CALLS:
            raise RuntimeError("provider call authorization exceeded")
        if tranche.counters.serialized_tokens > INITIAL_MAXIMUM_SERIALIZED_TOKENS:
        ...
        result = {
            ...
            "schema": "trellis-clean-whole-lifecycle-tranche-result-v0",
            "serialized_tokens": tranche.counters.serialized_tokens,
        }
        write_json(run_root / "TRANCHE_RESULT.json", result)
