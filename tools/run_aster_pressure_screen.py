...
SCOPE = "aster_provenance_relational_pressure_screen_v0"
...
CONTRACT = ROOT / "ASTER_PRESSURE_SCREEN_CONTRACT.json"
MODEL_LOCK = ROOT / "ASTER_MODEL_PROFILE_LOCK.json"
...
def base_messages(world: AsterWorld) -> list[dict[str, str]]:
    return [
        ...
        {"role": "user", "content": (TASK / "TASK.md").read_text(encoding="utf-8")},
        {
            ...
            + "No semantic-maintenance output is present during pressure screening. Both future configurations share this ordinary action surface and exact task work.\n\n"
...
def main() -> int:
    ...
    try:
        ...
        tokenizer = LiveTokenizer()
        world = AsterWorld(TASK, run_root / "trajectory")
        ledger = ResultLedger()
        ...
        for actor_call in range(1, MAX_CALLS + 1):
            ...
            if prompt_tokens > PROMPT_LIMIT:
                ...
                relief_source_result_ids = [
                    result_id
                    for result_id in relief_result_ids
                    if ledger.get(result_id).result_kind == "source_observation"
                ]
                if not relief_source_result_ids:
                    ineligible.append("relief_externalizes_no_source_observation")
                boundary = {
                    "schema": "aster-authentic-interaction-pressure-boundary-v0",
                    "task_id": TASK_ID,
                    ...
                    "counterfactual_externalized_source_result_ids": relief_source_result_ids,
                    "counterfactual_relief_prompt_tokens": relief_prompt_tokens,
                }
                write_json(run_root / "PRESSURE_BOUNDARY.json", boundary)
                if not ineligible:
                    terminal = "authentic_interaction_pressure"
                elif any(
                    item in {
            ...
            schema = action_json_schema(
                ...
                schema_name="aster_common_actor_action_v0",
            )
        ...
        result = {
            ...
            "pressure_qualified": terminal == "authentic_interaction_pressure",
