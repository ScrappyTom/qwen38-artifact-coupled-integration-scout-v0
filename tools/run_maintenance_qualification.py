...
RUN_ID = "2026-08-25-cedar-transfer-expression-qualification-v0"
SCOPE = "cedar_transfer_expression_qualification_v0"
MAX_CALLS = 4
...
def main() -> int:
    ...
    try:
        ...
        for offset, case in enumerate(build_action_cases(ROOT), 1):
            ...
            rows.append(row)
            write_json(call_root / "RESULT.json", row)
        result = {"schema": "cedar-transfer-expression-result-v0", "run_id": RUN_ID, "freeze_commit": git_commit(), "model_calls": len(rows), "passed": len(rows) == MAX_CALLS and all(row["accepted"] for row in rows), "cases": rows, "measured_actor_authorized": False}
