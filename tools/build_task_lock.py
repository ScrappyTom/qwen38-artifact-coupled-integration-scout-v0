...
from tools.materialize_cedar_world import SPECS, document


TASK = ROOT / "task"
SOURCE_ROOT = TASK / "cedar_sources"
...
def main() -> int:
    ...
    for ordinal, spec in enumerate(SPECS, start=1):
        ...
        custody.append(
            {
                **row,
                "origin": "deterministic_synthetic_cedar_valley_world_v0",
                "generator": "tools/materialize_cedar_world.py",
    ...
    write_json(
        ...
        {"schema": "cedar-source-catalog-v0", "sources": sources},
    )
    governed = [
        "ACTIONS.md",
        "EVALUATOR.json",
        ...
        *[f"cedar_sources/{spec.filename}" for spec in SPECS],
    ]
    write_json(
        TASK / "TASK_SOURCE_LOCK.json",
        {
            "schema": "cedar-task-source-lock-v0",
            "task_id": TASK_ID,
            "world_origin": "deterministic_synthetic_cedar_valley_world_v0",
