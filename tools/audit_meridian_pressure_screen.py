...
from reactive_runtime.meridian_world import MeridianWorld  # noqa: E402
...
from tools import run_meridian_pressure_screen as runner  # noqa: E402
from tools.offline_tokenizer import OfflineTokenizer  # noqa: E402


TASK = ROOT / "task_meridian"
AUDIT_NAME = "MERIDIAN_PRESSURE_SCREEN_AUDIT.json"
HANDOFF_NAME = "MERIDIAN_PRESSURE_BOUNDARY_HANDOFF.json"
FREEZE_COMMIT = "467ccc0d032af217b969a31315ee41005dbe2167"
...
def audit(repository_root: Path = ROOT, *, write_outputs: bool = True) -> dict[str, Any]:
    ...
    if failures:
        return {
            "schema": "meridian-pressure-screen-audit-v0",
    ...
    expected_freeze = {
        ...
        "task_source_lock_sha256": sha256_file(TASK / "TASK_SOURCE_LOCK.json"),
        "model_profile_lock_sha256": sha256_file(root / "MERIDIAN_MODEL_PROFILE_LOCK.json"),
        "screen_contract_sha256": sha256_file(root / "MERIDIAN_PRESSURE_SCREEN_CONTRACT.json"),
    }
    ...
    with tempfile.TemporaryDirectory() as replay_temporary:
        replay_world = MeridianWorld(TASK, Path(replay_temporary))
    ...
    if boundary.get("schema") != "meridian-authentic-pressure-boundary-v0":
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as activation_temporary:
            activation_world = MeridianWorld(TASK, Path(activation_temporary))
    ...
    if write_outputs:
        ...
        if audit_value["passed"]:
            handoff = {
                ...
                "claim_limit": "This handoff qualifies one exact common pre-treatment pressure fork. It establishes neither source-local expression nor W0/L1 utility and authorizes no continuation.",
            }
