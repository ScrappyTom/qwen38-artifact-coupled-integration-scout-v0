...
RUN_ID = "2026-08-25-bluehaven-pressure-screen-v0"
SCOPE = "bluehaven_pressure_screen_v0"
...
CONTRACT = ROOT / "BLUEHAVEN_PRESSURE_SCREEN_CONTRACT.json"
...
def verify_task_lock() -> None:
    ...
    for row in lock.get("files", []):
        ...
        if not path.is_file() or sha256_file(path) != row.get("sha256"):
            raise RuntimeError(f"Bluehaven task lock mismatch: {row.get('path')}")
...
def counterfactual_relief(
    *,
    ...
    """Audit the complete frozen first-fit pass without mutating live state."""
...
def main() -> int:
    ...
    world: ArchitectureWorld | None = None
    ...
    try:
        ...
        tokenizer = LiveTokenizer()
        world = ArchitectureWorld(TASK, run_root / "trajectory")
        ...
        for actor_call in range(1, MAX_CALLS + 1):
            ...
            if prompt_tokens > PROMPT_LIMIT:
                ...
                boundary = {
                    "schema": "bluehaven-authentic-pressure-boundary-v0",
                    "task_id": TASK_ID,
            ...
            try:
                ...
                if (
                    ...
                    pending_text = render_action_rejection(
                        ...
                        code=rejection,
                        message=(
                            "exact source result exceeded the frozen model-visible "
                            "token cap and remains audit-only"
                        ),
                        candidate_sha256=world.candidate_sha256,
                    )
                    result_record = None
