...
from tools.materialize_orchard_world import SPECS, document
...
def main() -> int:
    ...
    for spec in SPECS:
        ...
        sources.append(row)
        custody.append({**row, "origin": "deterministic_synthetic_orchard_biologics_world_v0", "generator": "tools/materialize_orchard_world.py"})
    write_json(TASK / "SOURCE_CATALOG.json", {"schema": "orchard-source-catalog-v0", "sources": sources})
    governed = [
        "ACTIONS.md",
        "VERIFICATION_ACTIONS.md",
        "EVALUATOR.json",
        "SOURCE_CATALOG.json",
        "SYSTEM.md",
        "TASK.md",
    ...
    write_json(
        ...
        {
            "schema": "orchard-task-source-lock-v0",
            "task_id": TASK_ID,
            "world_origin": "deterministic_synthetic_orchard_biologics_world_v0",
