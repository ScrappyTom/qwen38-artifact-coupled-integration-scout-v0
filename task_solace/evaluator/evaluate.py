...
def evaluate(candidate_root: Path) -> dict[str, Any]:
    ...
    rows.append(criterion("decision_heading_order", headings == CONFIG["decision_headings"], "exact ordered headings"))
    ...
    for gate, terms in CONFIG["semantic_term_gates"].items():
        ...
        rows.append(criterion(gate, not missing, "mechanical relation gate passed" if not missing else "missing terms: " + ", ".join(missing)))
    ...
    passed = not blocking
    return {
        "schema_version": "solace-water-evaluation-result-v0",
