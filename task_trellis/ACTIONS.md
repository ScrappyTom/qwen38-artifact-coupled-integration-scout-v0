# Trellis construction action contract

Return exactly one JSON action object and no prose.

```text
{"action":"read_source","source_id":"COUNCIL","start_line":1,"end_line":60}
...
{"action":"replace_evidence_ledger","content":"complete compact task-native evidence-to-requirement matrix"}
...
at most 160 lines and 12,000 exact source bytes. Incremental ledger and section
work is exact, versioned task state and remains available after source bodies
leave residency. `begin_verification` is valid only after the frozen mechanical
construction milestone. It changes lifecycle phase, not candidate bytes, and
