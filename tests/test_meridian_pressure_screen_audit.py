...
from tools.audit_meridian_pressure_screen import audit


ROOT = Path(__file__).resolve().parents[1]
RUN_ROOT = ROOT / "runs" / "2026-08-25-meridian-source-local-delta-pressure-screen-v0"


class MeridianPressureScreenAuditTests(unittest.TestCase):
    def test_exact_live_boundary_passes_independent_audit(self) -> None:
        ...
        self.assertEqual(69_784, result["serialized_tokens"])
        self.assertEqual("RESULT-006", result["pending_result_id"])
        self.assertEqual(23_628, result["ordinary_prospective_prompt_tokens"])
        self.assertEqual(2_636, result["overflow_tokens"])
        self.assertEqual(["RESULT-001"], result["positive_relief_result_ids"])
        self.assertEqual(20_774, result["positive_relief_after_tokens"])
    ...
    def test_pressure_history_is_six_valid_two_source_batches(self) -> None:
        ...
        for row, source_ids in zip(trace, expected, strict=True):
            ...
            self.assertTrue(all(item["end_line"] == 70 for item in requests))
    ...
    def test_handoff_binds_exact_audited_fork_and_authorizes_nothing(self) -> None:
        handoff = json.loads(
            (ROOT / "MERIDIAN_PRESSURE_BOUNDARY_HANDOFF.json").read_text(encoding="utf-8")
        )
        receipt = json.loads(
            (ROOT / "MERIDIAN_PRESSURE_SCREEN_AUDIT.json").read_text(encoding="utf-8")
        )
        ...
        self.assertEqual(receipt["positive_relief_result_ids"], handoff["positive_relief_result_ids"])
