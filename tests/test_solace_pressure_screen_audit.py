...
from tools.audit_solace_pressure_screen import audit


ROOT = Path(__file__).resolve().parents[1]
RUN_ROOT = ROOT / "runs" / "2026-08-26-solace-anchored-provenance-pressure-screen-v0"


class SolacePressureScreenAuditTests(unittest.TestCase):
    def test_exact_live_boundary_passes_independent_audit(self) -> None:
        ...
        self.assertEqual(63_731, result["serialized_tokens"])
        self.assertEqual("RESULT-006", result["pending_result_id"])
        self.assertEqual(21_211, result["ordinary_prospective_prompt_tokens"])
        self.assertEqual(219, result["overflow_tokens"])
        self.assertEqual(["RESULT-001"], result["positive_relief_result_ids"])
        self.assertEqual(18_595, result["positive_relief_after_tokens"])
        self.assertEqual(2_397, result["remaining_prompt_headroom_tokens"])
        self.assertFalse(result["expression_qualification_authorized"])
        self.assertFalse(result["measured_fork_authorized"])

    def test_history_is_six_valid_two_source_batches(self) -> None:
