...
def verify_orchard_pressure_handoff(root: Path) -> dict[str, Any]:
    root = root.resolve()
    handoff_path = root / "ORCHARD_PRESSURE_BOUNDARY_HANDOFF.json"
    audit_path = root / "ORCHARD_PRESSURE_SCREEN_AUDIT.json"
    handoff = _load(handoff_path)
    audit = _load(audit_path)
    run_root = root / str(handoff.get("run_root", ""))
    failures: list[str] = []
    expected = {
        "schema_version": "orchard-pressure-boundary-handoff-v0",
    ...
    bindings = {
        ...
        "task_source_lock_sha256": root / "task_orchard" / "TASK_SOURCE_LOCK.json",
    }
    for key, path in bindings.items():
        if not path.is_file() or handoff.get(key) != sha256_file(path):
            failures.append(f"binding:{key}")
    if failures:
        raise RuntimeError(f"Orchard pressure handoff failed: {sorted(set(failures))}")
    return handoff
